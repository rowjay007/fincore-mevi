...
func NewSettlePaymentHandler(uow ports.UnitOfWork) *SettlePaymentHandler {
	return &SettlePaymentHandler{uow: uow}
}

func (h *SettlePaymentHandler) Handle(ctx context.Context, cmd SettlePayment) (*SettlePaymentResult, error) {
	if cmd.PaymentID == "" {
	...
	err := h.uow.WithTx(ctx, func(ctx context.Context, es ports.PaymentEventStore, ob ports.OutboxStore, proj ports.PaymentProjectionRepository) error {
		...
		ev, ok := changes[0].(domain.PaymentSettled)
		if !ok {
		...
		res = &SettlePaymentResult{PaymentID: cmd.PaymentID, Version: ver, Status: p.Status()}
		return nil
	})
	if err != nil {
		return nil, err
